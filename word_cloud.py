...
from nonebot.adapters.onebot.v11 import Bot, Event, unescape, MessageSegment, Message
...
def chat_word_cloud() -> Rule:
    async def _chat_word_cloud(bot: "Bot", event: "Event", state: T_State) -> bool:
        if (
            event.__getattribute__("message_type") == "private"
            or event.user_id in bot_qq
            or event.is_tome()
        ):
            return False
        else:
...
async def read_csv(csv_name: str, user_id: str = None, _type: str = None):
    ...
    if not os.path.exists("data"):
        os.mkdir("data")
    if not os.path.exists(f"data{os.sep}wordcloud_bot"):
        os.mkdir(f"data{os.sep}wordcloud_bot")
    if not os.path.exists(f"data{os.sep}wordcloud_bot{os.sep}dict{os.sep}"):
        os.mkdir(f"data{os.sep}wordcloud_bot{os.sep}dict{os.sep}")
    if not os.path.exists(f"data{os.sep}wordcloud_bot{os.sep}stopwords{os.sep}"):
        os.mkdir(f"data{os.sep}wordcloud_bot{os.sep}stopwords{os.sep}")
    ...
    with open(f"data{os.sep}{csv_name}", "r", encoding="utf-8-sig", newline="") as f:
        jieba.load_userdict(f"data{os.sep}wordcloud_bot{os.sep}dict{os.sep}mydict.txt")
        ...
        for x in csv_list:
            ...
            if user_id and not re.search(user_id, x[1]):
                continue
            if is_continue(_type=_type, data_row=x):
                continue
            x[2] = re.sub(
                "(https?|ftp|file)://[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]",
                "",
...
def is_continue(_type: str, data_row) -> bool:
    if _type and _type == "today":
        today = time.mktime(
            time.strptime(
                f"{time.strftime('%Y-%m-%d', time.localtime())} 0:0:0",
                "%Y-%m-%d %H:%M:%S",
            )
        )
...
async def handle_first_receive(
    ...
    try:
        words = await read_csv(
            csv_name=f"{event.group_id}.csv", user_id=str(event.user_id), _type=_type
        )
    except:
        words = []
    if words:
